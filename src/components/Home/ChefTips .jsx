import React, { useEffect, useState } from "react";
import TipCard from "./TipCard";

const ChefTips = () => {

  const [chefTips,setChefTips]= useState([]);
  useEffect(()=>{
    fetch('https://myapp-belalhossain22000.vercel.app/chefTips')
    .then(res => res.json())
    .then(data=>setChefTips(data))
  },[])
  const tips = [
    {
      id: 1,
      chef: {
        name: "Chef John Smith",
        image: "data:image/jpeg;base64,",
      },
      title: "The Secret to Perfect Pancakes",
      summary:
        "Learn how to make fluffy, golden pancakes every time with this simple recipe from Chef John.",
    },
    {
      id: 2,
      chef: {
        name: "Chef Jane Doe",
        image: "data:image/jpeg;base64,",
      },
      title: "How to Cook the Perfect Steak",
      summary:
        "In this post, Chef Jane reveals her tried-and-true method for cooking the perfect steak every time.",
    },
    {
      id: 3,
      chef: {
        name: "Chef Mike Lee",
        image: "data:image/jpeg;base64,",
      },
      title: "How to Make a Great Sauce",
      summary:
        "Sauces can make or break a dish! In this article, Chef Mike shares his secrets for creating delicious, well-balanced sauces.",
    },
  ];
  return (
    <div>
      <section className="container mx-auto pb-20">
        <h2 className="text-3xl font-bold mb-6">Chef Tips</h2>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {chefTips.map((tip) => (
            <TipCard key={tip.id} tip={tip} />
          ))}
        </div>
      </section>
    </div>
  );
};

export default ChefTips;
